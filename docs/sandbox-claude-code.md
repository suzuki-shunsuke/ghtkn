...
`ghtkn info` tells you which side of this boundary you are on: it reports the installed version, and with the agent backend the running agent's version too, which stays behind until you [restart the agent](backend.md#restart-the-agent-after-upgrading-ghtkn).
