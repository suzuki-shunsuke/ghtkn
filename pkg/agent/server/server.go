...
// Server runs the ghtkn agent server.
type Server struct {
...
func (s *Server) refreshState() (bool, time.Duration) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return s.enableRefreshToken, s.refreshTokenTTL
}
