...
// newUnlockedServer returns a controller with an in-place disk store, as if it
...
// TestServer_handle_getSeeded verifies that a token seeded directly into the store
// is returned by GET.
func TestServer_handle_getSeeded(t *testing.T) {
	t.Parallel()
	c := newUnlockedServer(t)
...
// TestServer_handle_statusCounts verifies that STATUS reports the number of cached
// tokens.
func TestServer_handle_statusCounts(t *testing.T) {
	t.Parallel()
	c := newUnlockedServer(t)
...
// TestServer_handle_obsoleteClient verifies that a request below the minimum
...
func TestServer_handle_obsoleteClient(t *testing.T) {
	t.Parallel()
	c := newUnlockedServer(t)
...
// TestServer_handle_legacySetGet verifies that a legacy (protocol version 0,
...
func TestServer_handle_legacySetGet(t *testing.T) {
	t.Parallel()
	c := newUnlockedServer(t)
...
// TestServer_handle_setRejectedForCurrentClient verifies that SET is refused for a
...
func TestServer_handle_setRejectedForCurrentClient(t *testing.T) {
	t.Parallel()
	c := newUnlockedServer(t)
	const token = `{"access_token":"abc","expiration_date":"2999-01-01T00:00:00Z"}`
...
// TestServer_handle_legacyGetMissing verifies that a legacy GET miss returns
...
func TestServer_handle_legacyGetMissing(t *testing.T) {
	t.Parallel()
	c := newUnlockedServer(t)
...
// TestServer_handle_legacyNoRefresh verifies that even when the agent was unlocked
...
// The version-1 refresh path itself is covered by TestServer_handleGet_refresh.
func TestServer_handle_legacyNoRefresh(t *testing.T) {
	t.Parallel()
...
// TestServer_handle_obsoleteAgent verifies that a request with a protocol version
// newer than the agent's (the agent is out of date) is rejected before any dispatch.
func TestServer_handle_obsoleteAgent(t *testing.T) {
	t.Parallel()
	c := newUnlockedServer(t)
...
// TestServer_handle_locked verifies that a locked agent refuses GET and reports
// locked in STATUS.
...
// TestServer_handle_disk drives GET against a disk-backed encrypted store and
// verifies the token round-trips through encryption and disk persistence.
func TestServer_handle_disk(t *testing.T) {
	t.Parallel()
	c := newUnlockedServer(t)
...
// TestServer_handle_undecryptable verifies that a token persisted under a
...
func TestServer_handle_undecryptable(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
