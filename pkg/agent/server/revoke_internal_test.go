...
// TestServer_handleRevoke verifies that REVOKE revokes the stored tokens' raw access
// tokens in one batch, reports no failures, and deletes the stored copies.
func TestServer_handleRevoke(t *testing.T) {
	t.Parallel()
...
// TestServer_handleRevoke_noToken verifies that a client with no stored token is
// silently skipped: only the seeded token is revoked and the response reports no
// failures.
func TestServer_handleRevoke_noToken(t *testing.T) {
	t.Parallel()
	c := newUnlockedServer(t)
	rev := &fakeRevoker{}
	c.revoker = rev
...
// TestServer_handleRevoke_revokeFails verifies that when the batch revoke fails, every
// attempted client ID is reported in RevokeFailed, no cleanup failure is reported, and the
// stored tokens are left in place.
func TestServer_handleRevoke_revokeFails(t *testing.T) {
	t.Parallel()
	c := newUnlockedServer(t)
