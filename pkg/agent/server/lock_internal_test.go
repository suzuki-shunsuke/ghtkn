...
// TestServer_handle_lock verifies the LOCK command discards the in-memory data key:
...
// TestServer_handle_lock_reunlock verifies that LOCK keeps the key file, so a later
...
// TestServer_handleLock_idempotent verifies that locking an already-locked agent is a
// no-op success.
...
// TestServer_handleLock_stopsSweep verifies that LOCK stops the refresh-token sweep
...
// TestServer_startRefreshTokenSweep, which shows the same token IS swept without a
// lock.
