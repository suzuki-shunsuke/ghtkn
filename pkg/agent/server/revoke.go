...
func (s *Server) handleRevoke(ctx context.Context, req *agentapi.Request) *agentapi.Response {
	st := s.tokenStore()
	if st == nil {
		return &agentapi.Response{Error: agentapi.RespLocked}
	}

	tokens, attempted, revokeFailed := s.collectRevocableTokens(st, req.ClientIDs)
	if len(tokens) == 0 {
		return &agentapi.Response{OK: true, RevokeFailed: revokeFailed}
	}
	if err := s.revoker.Revoke(ctx, tokens); err != nil {
		...
		if s.logger != nil {
			slogerr.WithError(s.logger, err).Warn("revoke stored tokens")
		}
		return &agentapi.Response{OK: true, RevokeFailed: append(revokeFailed, attempted...)}
	}
	return &agentapi.Response{OK: true, RevokeFailed: revokeFailed, CleanupFailed: s.deleteRevoked(st, attempted)}
}
...
func (s *Server) collectRevocableTokens(st *tokenstore.Store, clientIDs []string) (tokens, attempted, revokeFailed []string) {
...
func (s *Server) deleteRevoked(st *tokenstore.Store, clientIDs []string) []string {
	var cleanupFailed []string
	for _, clientID := range clientIDs {
		if err := st.Delete(clientID); err != nil {
			if s.logger != nil {
				slogerr.WithError(s.logger, err).Warn("delete a revoked token", "client_id", clientID)
			}
