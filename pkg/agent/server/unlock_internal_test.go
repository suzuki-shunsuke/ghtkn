...
// TestServer_handle_unlock verifies the UNLOCK command loads the key and unlocks
// the agent.
...
// TestServer_handle_unlock_enableRefresh verifies that UNLOCK binds refresh-token
...
// TestServer_handle_unlock_refreshUnsupportedOS verifies the agent refuses an UNLOCK
...
// TestServer_handle_unlock_capsRefreshTokenTTL verifies the server clamps a
...
func TestServer_handle_unlock_capsRefreshTokenTTL(t *testing.T) {
	t.Parallel()
...
// TestServer_handle_unlock_orphanTokens verifies that unlocking with a freshly
...
func TestServer_handle_unlock_orphanTokens(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
...
// TestServer_handle_unlock_stripsRefreshWhenDisabled verifies that a confirmed unlock
...
// TestServer_handle_unlock_pendingRefreshRemoval).
func TestServer_handle_unlock_stripsRefreshWhenDisabled(t *testing.T) {
	t.Parallel()
...
// TestServer_handle_unlock_pendingRefreshRemoval verifies that a disabled unlock does
...
func TestServer_handle_unlock_pendingRefreshRemoval(t *testing.T) {
	t.Parallel()
	keyFile := filepath.Join(t.TempDir(), "key")
	tokenDir := t.TempDir()
...
// TestServer_handle_unlock_confirmedRefreshRemoval verifies that a disabled unlock
// carrying the confirmation flag unlocks the agent and strips the refresh token.
func TestServer_handle_unlock_confirmedRefreshRemoval(t *testing.T) {
	t.Parallel()
	keyFile := filepath.Join(t.TempDir(), "key")
	tokenDir := t.TempDir()
...
// TestServer_handle_unlock_noPromptWhenRefreshExpired verifies that an already-expired
// refresh token is worthless, so a disabled unlock strips it without a confirmation prompt.
func TestServer_handle_unlock_noPromptWhenRefreshExpired(t *testing.T) {
	t.Parallel()
	keyFile := filepath.Join(t.TempDir(), "key")
	tokenDir := t.TempDir()
