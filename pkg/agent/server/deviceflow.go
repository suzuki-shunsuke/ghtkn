...
// that sets it.
func (s *Server) deviceFlow(clientID string) (*deviceFlowState, bool) {
	s.statusMu.Lock()
...
func (s *Server) clearDeviceFlow(clientID string) {
	s.statusMu.Lock()
	defer s.statusMu.Unlock()
...
func (s *Server) failDeviceFlow(clientID string) {
	s.statusMu.Lock()
	defer s.statusMu.Unlock()
	if st, ok := s.status[clientID]; ok {
...
func (s *Server) startDeviceFlow(ctx context.Context, logger *slog.Logger, clientID string, enableRefreshToken bool) (*deviceFlowState, error) {
	...
	s.statusMu.Lock()
	if existing, ok := s.status[clientID]; ok {
		s.statusMu.Unlock()
		return existing, nil
	}
	s.status[clientID] = state
	s.statusMu.Unlock()
	...
	go func() {
		...
		if err := s.pollAndStore(ctx, logger, clientID, deviceCodeResp, enableRefreshToken); err != nil {
			slogerr.WithError(logger, err).Error("the device flow did not store a token", "client_id", clientID)
			s.failDeviceFlow(clientID)
			return
		}
		s.clearDeviceFlow(clientID)
	}()
...
func (s *Server) pollAndStore(ctx context.Context, logger *slog.Logger, clientID string, deviceCode *deviceflow.DeviceCodeResponse, enableRefreshToken bool) error {
	token, err := s.client.Poll(ctx, logger, clientID, deviceCode, nil)
	if err != nil {
	...
	raw, err := s.encodeToken(token, enableRefreshToken)
	if err != nil {
...
func (s *Server) encodeToken(token *deviceflow.AccessToken, enableRefreshToken bool) (json.RawMessage, error) {
	now := time.Now()
	ac := &storedToken{
