...
// feature is unsupported (see server.RefreshTokenSupported). The agent refuses such an
...
func checkRefreshTokenSupported(enableRefreshToken bool, goos string) error {
	if enableRefreshToken && !server.RefreshTokenSupported(goos) {
...
func parseRefreshTokenTTL(s string) (time.Duration, error) {
	...
	if d > server.MaxRefreshTokenTTL {
