...
func seedToken(t *testing.T, c *Controller, id string, expiration time.Time) {
	t.Helper()
...
// TestController_startRefreshTokenSweep verifies the periodic half of the sweep: the
...
func TestController_startRefreshTokenSweep(t *testing.T) {
	t.Parallel()
	synctest.Test(t, func(t *testing.T) {
		c := newUnlockedController(t)
