...
- The agent sweeps tokens unused past a TTL (default 7 days, `--refresh-token-ttl`, max ~6 months), deleting the whole encrypted file to avoid holding long-lived refresh tokens for rarely used apps. The TTL takes only `d`/`w`/`m` units (`m` = 30-day month; `h`/`m`/`s` are rejected), and applies only with `--enable-refresh`.
- Tradeoff: while the agent is unlocked, enabling refresh widens the window in which a same-user process can obtain access tokens - in particular an idle app's token stays reachable for up to the TTL. The one-week default keeps that far below a refresh token's own six-month life while surviving a long weekend, so most people need no further action; if you want a smaller window you can shorten the TTL or lock the agent when idle (`ghtkn agent lock`), and the extra-cautious can revoke an unused app's token with `ghtkn revoke`.
