...
- The agent sweeps tokens unused past a TTL (default 1 week, `--refresh-token-ttl`, max ~6 months), deleting the whole encrypted file to avoid holding long-lived refresh tokens for rarely used apps. The TTL takes only `d`/`w`/`m` units (`m` = 30-day month; `h`/`m`/`s` are rejected), and applies only with `--enable-refresh`.
